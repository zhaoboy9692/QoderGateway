...
- 账号池支持持久化备注、行内编辑，并可按账号名、UID 或备注搜索。
- 中文与英文文档，支持搜索、目录导航和语言切换。

账号通过 PAT 或账号 JSON 导入，支持导出备份；开启自动调度后轮流选择有个人或团队额度的账号。本地登录扫描已移除。自动注册功能、独立注册工具及其浏览器依赖已移除。
...
首次进入**账号池**时自动加载额度；切换页面或点击“查看限额”复用本次页面会话的结果，不重复查询。刷新整个浏览器页面后会重新自动加载一次。

- **查看限额**：定位到额度面板。**刷新 / 刷新状态**：重新查询启用账号，并同步套餐和重置日期。
- 每个账号右侧的**刷新**：仅更新该账号。
